...
def progress_bar(files, desc='Processing', total=None):
    if total is None:
        total = len(files)
    return tqdm(files, desc=desc, total=total, dynamic_ncols=True)
...
def main():
    ...
    with futures.ThreadPoolExecutor(**kwargs) as executor:
        ...
        for root, _, files in os.walk('images'):
            for basename in progress_bar(files, 'Queueing'):
                if not basename.endswith('.jpg'):
                    continue
                filename = f'{root}/{basename}'

                tasks.add(executor.submit(process_file, filename))

        for _ in progress_bar(futures.as_completed(tasks), total=len(tasks)):
            pass
